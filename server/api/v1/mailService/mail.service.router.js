...
router.post('/confirmationMail', (req,res) => {
        ...
        transporter.sendMail(mailOptions, (err, info) => {
            if(err) {
                ...
                res.status(200).send('Confirmation mail has been sent successfully!');
            }
        });
    
});

router.post('/deliveryMail', (req,res) => {
        ...
        transporter.sendMail(mailOptions, (err, info) => {
            if(err) {
                if(config.logging) {
                    logger.error("Error occurred while sending delivery mail: ", err);
                }
                console.log("Error occurred while sending delivery mail: ", err);
                res.status(500).send(err);
            } else {
                console.log('Delivery mail has been sent successfully!');
                res.status(200).send('Delivery mail has been sent successfully!');
            }
        });
        
});

router.post('/referralMail', (req,res) => {
    
        const receiversName = req.body.receiversName; /*receivers name is name of guy whose referral code have been used*/
        const referralCode = req.body.referralCode;
        const referralUsedBy = req.body.referralUsedBy;
        const to = req.body.to;
        ...
        transporter.sendMail(mailOptions, (err, info) => {
            if(err) {
                ...
                console.log('Referral mail has been sent successfully!');
                res.status(200).send('Referral mail has been sent successfully!');
            }
        });    
});
...
router.post('verificationMail', (req,res) => {
        ...
        const receiversName = req.body.receiversName;
        const to = req.body.to;
        const message= `<strong>Hello ${receiversName}</strong>, <br>Your registration is almost complete<br><br>
        <p> <br>
        Your friend ${referralUsedBy} has used your referral code ${referralCode}<br>
        <br>You can use coupon code KitchenAtNight30 to avail <strong>30% off upto 100 rupees</strong> on your next order.<br>
        <br>Keep referring to friends and earn more referral coupon.<br>
